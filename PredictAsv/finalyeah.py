...
portnumber = "http://192.168.1.4:5502"  # 바꾸세요 ^^7

# log찍으려고 하는것(난이도 설정가능함)
logging.basicConfig(level=logging.INFO)

# model폴더에서 tokenizer_pre.pickle와 model_pre.h5 그리고 mfcc.h5불러오기
#with open("C:/model/tokenizer_pre.pickle", "rb") as f:
 #   tokenizer1 = pk.load(f)
#model1 = load_model("C:/model/model_pre.h5")
#model2 = load_model("C:/model/mfcc.h5")
...
# ModelUpdate - api
@api.route("/api/modelupdate/<int:idx>/<string:declaration>", methods=["GET"])
class HelloWorld(Resource):
    def get(self, idx, declaration):
        ...
        model.fit(train_seq, train_target, epochs=1, batch_size=32, validation_data=(val_seq, val_target))
...
class HelloWorld(Resource):
    def post(self, idx, declaration):
        ...
        if request.method == 'POST':
            ...
            notify_progress(idx, declaration, "20%", f"--{FileName}.replace")
            ...
            query = f"""INSERT INTO voicedata (idx, declaration, audio_file, content, disdata, created_date, modified_date, reroll, mfcc, admindata, persent ) VALUES ( '{idx}','{declaration}','{FileName}','{text_final}','{detect}',NOW(),NOW(),'null','{result_mfcc}',null,'{persent}')"""
