...
TEST_MODE = False #스프링에 연동할때 False
...
def notify_progress(user_id, declaration,progress, work):
...
class HelloWorld(Resource):
    def post(self, user_id, declaration):
        ...
        if request.method == 'POST':
            ...
            notify_progress(user_id,declaration,"0%","--Post")
            if TEST_MODE:
            ...
            notify_progress(user_id,declaration,"10%","--File_receive")
            
            FileName = wav_filename.replace(WAV_PATH, "")
            notify_progress(user_id, declaration, "20%", "--wav_filename.replace")
            
            cut_wav(wav_filename)
            notify_progress(user_id, declaration, "30%", "--cut_wav")
            
            data_list = get_datalist(wav_filename)
            notify_progress(user_id, declaration, "40%", "--get_datalist")
            
            stt_result_list = transcribe_audio(data_list)
            notify_progress(user_id, declaration, "50%", "--transcribe_audio")
            
            text_final = concatenate_texts(stt_result_list)
            notify_progress(user_id, declaration, "60%", "--concatenate_texts")
            
            prediction = predict1(text_final)
            notify_progress(user_id, declaration, "70%", "--voice_complete")
            
            X_data, y_data = wav_mfcc(wav_filename)
            notify_progress(user_id, declaration, "80%", "--wav_mfcc")
            
            result_mfcc = predict2(X_data, y_data)
            notify_progress(user_id, declaration, "90%", "--mfcc_complete")
            persent = prediction
            ...
            notify_progress(user_id, declaration, "100%", "--all_work_complete")
            ...
            query = f"""INSERT INTO voicedata (idx, declaration, audio_file, content, disdata, created_date, modified_date, reroll, mfcc, admindata, persent )
             VALUES ( '{user_id}','{declaration}','{FileName}','{text_final}','{prediction}',NOW(),NOW(),'null','{result_mfcc}',null,'{persent}')"""
